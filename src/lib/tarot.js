...
function generateOverview(cards) {
  ...
  let overview = `塔罗牌显示：`
  overview += `过去受到「${past.name}${past.isReversed ? '（逆位）' : ''}」的影响，${past.meaning.slice(0, 30)}...`
  overview += `当前状况对应「${present.name}${present.isReversed ? '（逆位）' : ''}」，${present.meaning.slice(0, 30)}...`
  overview += `未来趋势指向「${future.name}${future.isReversed ? '（逆位）' : ''}」，${future.meaning.slice(0, 30)}...`
